function params = setParametersC4_TC002()
% SETPARAMETERSC4_D6  Clavinet model parameters TC-002 prototype build
%
...
params.pitch.freq_Hz     = 263.5;%261.63;

%--- String (plain music-wire steel) --------------------------------------
params.string.L_m        = 0.495;%0.600;        % speaking length (anvil -> bridge)
params.string.d_m        = 0.6e-3;%0.229e-3;     % diameter (0.009")
params.string.rho_kgm3   = 7850;         % mass density, steel
params.string.E_Pa       = 2.00e11;      % Young's modulus, steel
params.string.sigma0_Hz  = 0.3;%1.0;          % freq-indep. damping        % tune
params.string.sigma1_s   = 3e-3;%5e-5;         % freq-dep. damping          % tune
...
params.pickups(1).x_m         = 0.045;%0.040;
params.pickups(1).aperture_m  = 0.030;
params.pickups(2).name        = 'tangent';
params.pickups(2).x_m         = 0.160;%0.120;
...
params.excite.x_m            = 0.490;%0.060;    % strike point (10% from bridge)
params.excite.width_m        = 0.01;%0.005;    % raised-cosine half-width
params.excite.v_peak_mps     = 1.0;      % peak initial velocity
