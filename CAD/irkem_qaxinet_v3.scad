...
color("blue",0.3) translate([-100-250,-50-20,0]) desktop();
...
//customGrid(1200,10);
//customGridR(1200,100);
//customGridYZ(1200,10);
//pickups();
//damper();
//controls();
...
module pinBlock(){
    translate([-300,45,-30]) cube([kSpc*nStr+100,45,30]);
}
...
module desktop(){
    translate([0,-260,-2*25.4]) cube([1530,760,2*25.4]);
}
...
module strings(){
    for (i = [1:1:(nStr)]) {
        ...
        //translate([i*kSpc,0,0]) rotate([0,0,0]) cylinder(40,2,2);
        translate([i*kSpc,0,0]) rotate([0,90,-skew+180]) cylinder(300,1,1);
        kL1 = 220;
        if (i%2==1){
        color("gray",1) translate([i*kSpc-kL1*cos(15),kL1*sin(15),-48]) rotate([0,0,0]) cylinder(64,3.5,3.5);
        }
        kL2 = 307;
        if (i%2==0){
        color("gray",1) translate([i*kSpc-kL2*cos(15),kL2*sin(15),-48]) rotate([0,0,0]) cylinder(64,3.5,3.5);
        }
    }
}
