...
module controls(){
    ...
    //color("ivory",1.0) translate([965+70,50-4-12.7,0]) cylinder(10,12.7,12.7);
...
module harp(){
    ...
    translate([0,-25,0]) rotate([0,0,-skew]) translate([-100-120-25,-44,-44+3]) cube([harpX,44,3]);
    //translate([1020,50-50,-44+3]) rotate([0,0,-90-skew]) translate([0,0,0]) cube([harpY+0+120-47-48,44,3]);
    translate([1020+70,50-50+20+5,-44+3]) rotate([0,0,-90-skew]) translate([0,0,0]) cube([harpY,44,3]);
}
...
module strings(){
    for (i = [1:1:(nStr)]) {
        translate([i*kSpc,0,0]) rotate([0,90,-skew]) cylinder(980-i*kSpc*cos(skew),1,1);
...
module pickups(){
    translate([950,-50,-35+5]) rotate([0,0,-(90+skew)]) cube([187,20,30]);
    translate([910,-50,-35+5]) rotate([0,0,-(90+skew+30)]) cube([187,20,30]);
}
