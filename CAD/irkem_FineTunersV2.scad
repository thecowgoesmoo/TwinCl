...
fineTunerAssy();
//difference(){
//    translate([0,-5,-16]) cube([kSp*nKeys+20,8,22]);//cube([180,8,22]);//cube([180,5,22]);
//    fineTunerAssy();
//}
...
for (i = [1:1:nKeys]) {
    ...
    translate([spR+3.7+kSp*i-13.75,10,0]) rotate([90,0,0]) cylinder(40,3.6+prEps,3.6+prEps);
...
module daHolz(){
    for (i = [1:1:nKeys]) {
        translate([16.5+i*kSp-kSp-2.5,10,-20]) cylinder(100,5,5);
        //translate([16.5+i*kSp-kSp-2.5,10,-20+0]) cylinder(100,8,5);
    }
}

module wormAsy(){
    translate([0,0,0]) rotate([90,0,0]) cylinder(spH,spR+prEps,spR+prEps
    );
    translate([0,0,-spR-wrmR+thrH]) rotate([0,90,0]) translate([0,-2.5,-wrmH/2]) cylinder(wrmH,wrmR+prEps,wrmR+prEps);
}