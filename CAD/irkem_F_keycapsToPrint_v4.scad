include <irkem_hammerTips_v1.scad>
...
color("ivory") difference(){
    translate([k*oct+oct*3/7+xO-2.25,-wL+yO,0+zO]) cube([oct*1/7-gap+3,wL+bL,kH]);
    translate([k*oct+oct*6/12-2+xO,0+yO,0+zO]) cube([4+oct*(1/7-1/12)-gap+1,bL,kH]);
}
...
for (i = [5]){//,2,4,5,7,9,11]){//[0:1:(n_keys-1)]){
